...
function arena_text_draw(x_position, y_position, x_scale, y_scale)
{
	...
	for(i = 0; i < arena_width; i++){
		for(j = 0; j < arena_height; j++){
			if(check_value(i, j, "Empty")){
				draw_text_transformed_color(grid_draw_location, arena_pos_value_str, x_scale, y_scale, 0, empty_text_color_4x, 100);
			}
